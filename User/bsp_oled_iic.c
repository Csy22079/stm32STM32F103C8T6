...
uint8_t OLED_CheckDevice(uint8_t _Address)
{
	...
	Soft_IIC_Start();		/* 发送启动信号 */

	Soft_IIC_Write_Byte(_Address|OLED_I2C_WR);/* 发送设备地址 */
	ucAck = Soft_IIC_Wait_Ack();	/* 检测设备的ACK应答 */

	Soft_IIC_Stop();			/* 发送停止信号 */
...
void I2C_WriteByte(uint8_t addr,uint8_t data){
	...
	Soft_IIC_Start();//开启I2C总线
	
	/* 发送设备地址+读写控制bit（0 = w， 1 = r) bit7 先传 */
	Soft_IIC_Write_Byte(OLED_ADDRESS|OLED_I2C_WR);
	...
	Soft_IIC_Write_Byte(addr);//发送寄存器地址
